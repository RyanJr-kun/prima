...
<div class="card">
  ...
  <div class="card-header sm-pt-4 sm-pb-0 border-bottom">
    <div class="row">
        ...
        <div class="col-6 text-end">
            <button class="btn btn-primary add-new" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddProdi" id="btnCreate">
                ...
                <div class="d-flex justify-content-start align-items-center user-name">
                    <div class="avatar-wrapper">
                        <div class="avatar avatar-sm me-3">
                            <span class="avatar-initial rounded-circle bg-label-primary">{{ substr($prodi->kaprodi->name, 0, 2) }}</span>
                        </div>
                    </div>
                    <div class="d-flex flex-column">
                        <p class="mb-0">{{ $prodi->kaprodi->name }}</p>
                        <small>NIDN : {{ $prodi->kaprodi->nidn }}</small>
                    </div>
                </div>
            </td>
            <td>
                <div class="d-flex align-items-center">
                    ...
                    <form action="{{ route('master.program-studi.destroy', $prodi->id) }}" method="POST" class="d-inline delete-form">
                        @csrf
                        @method('DELETE')
                        <a href="javascript:;" 
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasAddProdi" aria-labelledby="offcanvasAddProdiLabel">
    ...
    <div class="offcanvas-body mx-0 grow-0 p-6 h-100">
      ...
      <form class="add-new-prodi pt-0" id="addNewProdiForm" action="{{ route('master.program-studi.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            <div class="mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="add-name-prodi" placeholder="Teknologi Rekayasa Perangkat Lunak" name="name" value="{{ old('name') }}" />
            @error('name')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            @error('jenjang_id') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Lama Studi (Jumlah Semester)</label>
            <input type="number" id="lamaStudi"
                class="form-control" 
                name="lama_studi" 
                placeholder="Contoh: 6 untuk D3, 8 untuk S1"
                required>
        ...
        <div class="mb-3">
            ...
            <select name="kaprodi_id" id="kaprodi_id" class="form-select select2">
                ...
                @endforeach
                @error('kaprodi_id') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
            </select>
        </div>
