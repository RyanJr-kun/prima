                                ...
                                <div class="d-flex justify-content-start align-items-center user-name">
                                    <div class="avatar-wrapper">
                                        <div class="avatar avatar-sm me-3">
                                            <span
                                                class="avatar-initial rounded-circle bg-label-primary">{{ substr($user->name, 0, 2) }}</span>
                                        </div>
                                    </div>
                                    <div class="d-flex flex-column">
                                        <a href="#" class="text-body text-truncate"><span
                                                class="fw-medium">{{ $user->name }}</span></a>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    @if (!empty($user->signature_path))
                                        <a href="{{ asset('storage/' . $user->signature_path) }}" target="_blank"
                                        ...
                                        <a href="{{ asset('storage/' . $user->signature_path) }}"
                                            download="{{ $user->name }}_signature.png" target="_blank"
                                            class="text-body me-2" data-bs-toggle="tooltip" data-bs-offset="0,6"
                                            data-bs-placement="bottom" data-bs-html="true"
                                    ...
                                    <a href="javascript:;" class="text-body edit-record me-2" data-bs-toggle="offcanvas"
                                        data-bs-target="#offcanvasAddUser" data-id="{{ $user->id }}"
                                        data-name="{{ $user->name }}" data-username="{{ $user->username }}"
                                        data-email="{{ $user->email }}" data-nidn="{{ $user->nidn }}"
                                        data-roles='@json($user->getRoleNames())' data-status="{{ $user->status }}"
                ...
                <form class="add-new-user pt-0" id="addNewUserForm" action="{{ route('user.store') }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror"
                            id="add-user-fullname" placeholder="John Doe" name="name" value="{{ old('name') }}" />
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('username') is-invalid @enderror"
                            id="add-user-username" placeholder="johndoe" name="username"
                            value="{{ old('username') }}" />
                        @error('username')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nidn') is-invalid @enderror" id="add-user-nidn"
                            placeholder="617892733" name="nidn" value="{{ old('nidn') }}" />
                        @error('nidn')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('email') is-invalid @enderror"
                            id="add-user-email" placeholder="john.doe@example.com" name="email"
                            value="{{ old('email') }}" />
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select name="userRole[]" id="user-role"
                            class="form-select select2 @error('userRole') is-invalid @enderror" multiple
                            data-placeholder="Pilih Role">
                            @foreach ($roles as $role)
                                <option value="{{ $role }}"
                                    {{ collect(old('userRole'))->contains($role) ? 'selected' : '' }}>
                                    {{ $role }}</option>
                            @endforeach
                        </select>
                        @error('userRole')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="password" class="form-control @error('password') is-invalid @enderror"
                            id="add-user-password" name="password" placeholder="********" />
                        <small class="text-muted" id="password-help" style="display:none">Biarkan kosong jika tidak ingin
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" class="form-control @error('signature_path') is-invalid @enderror"
                            id="signature_path" name="signature_path" accept="image/png" />
...
@section('page-script')
    <script type="module">
        ...
        const initSelect2 = () => {
            ...
            if (typeof $ !== 'undefined' && $.fn.select2) {
                ...
                $('#offcanvasAddUser .select2').each(function() {
                    const $this = $(this);
                    $this.select2({
                        placeholder: $this.data('placeholder') || "Pilih...",
                        allowClear: true,
                        dropdownParent: $('#offcanvasAddUser'), // <--- INI KUNCINYA
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Handler untuk tombol Sync SIAKAD
            const btnSync = document.getElementById('btnSyncSiakad');
            if (btnSync) {
                btnSync.addEventListener('click', function() {
                    Swal.fire({
                        title: 'Konfirmasi Sinkronisasi',
                        text: "Proses ini akan mengambil data dari SIAKAD. Lanjutkan?",
                        icon: 'question',
                        ...
                        if (result.isConfirmed) {
                            Swal.fire({
                                ...
                                text: 'Mohon tunggu, sedang mengambil data dari SIAKAD.',
                            ...
                            document.getElementById('formSyncSiakad').submit();
                        }
                    });
                });
            }
